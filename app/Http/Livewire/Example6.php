...
class Example6 extends Component
{
    ...
    public function mount()
    {
        ...
        //query total score
        $db_totalscore = DB::table('total_current')->where('users_id', $this->ids)->select('TotalCurrent', 'TotalCurrent2')->get()->first();
        $this->checkTotalScore = $db_totalscore->TotalCurrent;
        $this->checkTotalScore2 = $db_totalscore->TotalCurrent2;
    ...
    public function trueResult()
    {
        $checkScore = DB::table('questions_flow')->where('users_id', $this->ids)->where('QuizGroup', $this->checkQuizGroup)
            ->update([
                'CurrentScore' => $this->checkCurrentScore + 100,
                'created_at' => now(),
                'updated_at' => now()
            ]);
        ...
        $total = DB::table('total_current')->where('users_id', $this->ids)
            ->update([
                'TotalCurrent2' => $this->checkTotalScore2 + 100,
            ]);
        $total_score = DB::table('users')->where('id', $this->ids)
            ->update([
                'TotalScore' => $this->checkTotalScore + $this->checkTotalScore2,
            ]);


        $this->a = 0;
        if ($this->checkQuizGroup + $this->a === 5) {
    ...
    public function falseResult()
    {
        ...
        $total = DB::table('total_current')->where('users_id', $this->ids)
            ->update([
                'TotalCurrent2' => $this->checkTotalScore + 0,
            ]);
    ...
    public function render()
    {
        ...
        $this->displayCurrentScore = $this->checkTotalScore2;

        return view('livewire.example6');
    }
}
